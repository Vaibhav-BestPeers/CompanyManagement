class ChattingController < ApplicationController
    def index
        @user = User.find(params[:userr_id])
        @user_all = User.all
    end
    ...
    def show 
      ...
      @chats = Chat.where(user_id: params[:userr_id], to: params[:id] ).or(Chat.where(user_id: params[:id],to: params[:userr_id])).order(id: :asc)
    ...
    def create
      ...
      if @chat.save 
         redirect_to userr_chatting_path(@user.id,@friend.id), vaibhav_flash: 'Chat Succesfully Sent'
      else
         byebug
          render('show')
      end
